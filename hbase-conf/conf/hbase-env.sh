...
# JAVA_HOME is normally exported by the artifact framework (env.sh sets it
# from DGC_JDK before each launch). The fallback below resolves to a sibling
# of the HBase install (same parent dir as hbase-test/, where the prebuilt
# DGC JDK should live).
export JAVA_HOME=${JAVA_HOME:-${HBASE_HOME}/../../jdk17-snic-gc-prebuilt/jdk}
