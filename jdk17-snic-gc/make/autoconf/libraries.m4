...
[
  ...
  if test "x$OPENJDK_TARGET_OS" = xlinux || test "x$OPENJDK_TARGET_OS" = xaix; then
    # 添加原有的库
    BASIC_JVM_LIBS="$BASIC_JVM_LIBS -lpthread -lortools -lortools_flatzinc -libverbs"
    
    # 添加所有.a文件（带完整路径）
    if test -n "$ORTOOLS_LIB_PATH" && test -d "$ORTOOLS_LIB_PATH"; then
      # 使用find命令获取所有.a文件并格式化为链接器选项
      for lib in `find "$ORTOOLS_LIB_PATH" -name '*.a' -printf '%f\n' | sed 's/^lib//;s/\.a$//'`; do
        BASIC_JVM_LIBS="$BASIC_JVM_LIBS -l$lib"
      done
    fi
  fi
  ...
  # Add OR-Tools library if enabled
  if test "x$ORTOOLS_TO_USE" = "xsystem" ; then
    JVM_LIBS="$JVM_LIBS $ORTOOLS_LIBS"
    BASIC_JVM_LIBS="$BASIC_JVM_LIBS $ORTOOLS_LIBS"
  fi
