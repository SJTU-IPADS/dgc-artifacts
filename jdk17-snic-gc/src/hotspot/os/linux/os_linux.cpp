...
bool os::create_thread(Thread* thread, ThreadType thr_type,
...

  {
    ...
    if (ret == 0) {
      ...
      // if(strlen(GCThreadsCores) > 0 && thr_type == cgc_thread){
      ...
      //       }
      //     }
      //     log_info(os, thread)("Successfully set gc thread affinity to %s", cpu_numbers_str.c_str());
      //   }
      // }
      
      if(strlen(MutatorCores) > 0 && (thr_type == java_thread || thr_type == cgc_thread) ){
        log_info(os, thread)("MutatorCores: %s", MutatorCores);
        cpu_set_t cpuset;
        CPU_ZERO(&cpuset);
        int mutator_core_len = strlen(MutatorCores); 
        int i = 0;
        int core_idx = 0;
        std::vector<int> cpu_numbers = {};
        while(i < mutator_core_len){
          if(MutatorCores[i] == ','){
            CPU_SET(core_idx, &cpuset);
            cpu_numbers.push_back(core_idx);
            core_idx = 0;
        ...
        ret = pthread_setaffinity_np(tid, sizeof(cpu_set_t), &cpuset);
        if (ret != 0) {
          log_warning(os, thread)("Failed to set java thread affinity - pthread_setaffinity_np failed (%d=%s)",
            ret, os::errno_name(ret));  
        } else {
          std::string cpu_numbers_str = "";
          for(unsigned int i = 0; i < cpu_numbers.size(); i++){
          ...
          log_info(os, thread)("Successfully set java thread affinity to %s", cpu_numbers_str.c_str());
        }
      }
