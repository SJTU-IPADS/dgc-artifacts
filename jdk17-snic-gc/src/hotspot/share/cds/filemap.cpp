    ...
    if(SnicGCHost){
      if(r==0){
        ...
        if (!SnicGCShareMemEnabled) {
          log_info(gc)("Mapping cds archive, with base=%p, sz=%ld", (void *)(si->mapped_base()), (p2i(si->mapped_end()) - p2i(si->mapped_base())));
          int mr_idx = Universe::add_mr((void *)(si->mapped_base()), (p2i(si->mapped_end()) - p2i(si->mapped_base()))); //archive_space_rs.size());
          int buffer[3];
          buffer[2] = mr_idx;
          Universe::send_rpc(5, 3*sizeof(int), buffer);
          Universe::recv_int_ack();
          log_debug(gc)("DGC LOG: CDS archive RPC 5 ack received.");
        } else {
          // TODO @lyh: impl of app cds.
          char* buffer = new char[1024];
          unsigned long long* message = (unsigned long long *)(buffer + 2 * sizeof(int));
          message[0] = (unsigned long long) (si->mapped_base());
          message[1] = (unsigned long long) (p2i(si->mapped_end()) - p2i(si->mapped_base()));
          log_debug(gc)("DGC LOG: send rpc 7 to share mem client");
          Universe::send_rpc(7, 2 * sizeof(int) + 2 * sizeof(unsigned long long), buffer);
        }
        /*end @lyh*/
      }
    }
