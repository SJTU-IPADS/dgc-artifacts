    ...
    LP64_ONLY({
        if (Metaspace::using_class_space()) {
          ...
          if (SnicGCHost && SnicGCShareMemEnabled) {
            log_debug(gc)("DGC LOG: rpc_8 case 1");
            if ((unsigned long long) (class_space_rs.base()) != 0x800000000) {
              char path[1024];
              // strcpy(path, SnicShmMemPath);
              strcpy(path, Universe::generate_virtual_node_shm_path());
              log_debug(gc)("DGC LOG: create virtual node shm, path %s", path);
              auto fd = shm_open(path, O_CREAT | O_RDWR, 0666);
              if (fd < 0) {
                log_error(gc)("DGC LOG: create virtual node shm failed");
                exit(0);
              }
              class_space_rs.remap_reserved_space_with_backup(path, fd);
              // // TEST ONLY
              // unsigned long long *region_data = (unsigned long long *)(class_space_rs.base() + class_space_rs.size());
              // region_data -= 1;
              // *(region_data) = 123456ULL;
              // log_debug(gc)("DGC LOG: test class space data at %p, %llu, space start:%p,end:%p", region_data, *(region_data), class_space_rs.base(), class_space_rs.end());
            }
          }
          /*end @lyh*/
          ...
          // /*start @lyh*/
          // if (SnicGCHost && SnicGCShareMemEnabled) {
          //   log_debug(gc)("DGC LOG: send rpc 11 to share mem client");
          //   char* buffer = new char[1024];
          //   unsigned long long *message = (unsigned long long *)(buffer + 2 * sizeof(int));
          //   message[0] = (unsigned long long)(CompressedKlassPointers::base());
          //   Universe::send_rpc(11, 2 * sizeof(int) + sizeof(unsigned long long), buffer);
          // }
          // /*end @lyh*/
        }
      });
...
char* MetaspaceShared::reserve_address_space_for_archives(FileMapInfo* static_mapinfo,
  ...
  if (use_windows_memory_mapping() && use_archive_base_addr) {
    ...
    archive_space_rs = total_space_rs.first_part(ccs_begin_offset, (size_t)archive_space_alignment);
    class_space_rs = total_space_rs.last_part(ccs_begin_offset);
    MemTracker::record_virtual_memory_split_reserved(total_space_rs.base(), total_space_rs.size(), ccs_begin_offset);
  }
...
MapArchiveResult MetaspaceShared::map_archive(FileMapInfo* mapinfo, char* mapped_base_address, ReservedSpace rs) {
  ...
  log_info(gc)("Mapping cds archive, with base=%p, sz=%ld, path=%s", (void *)(rs.base()), rs.size(), mapinfo->full_path());
