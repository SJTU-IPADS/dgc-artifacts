  ...
  if(SnicGCHost){
    if(result != NULL){
      ...
      if (!SnicGCShareMemEnabled) {
        // SnicGCRDMABatchFetchKlass: hwm is updated at post-linking in
        // InstanceKlass::link_class_impl (see rationale at that site).
        if (!SnicGCRDMABatchFetchKlass) {
          unsigned long long klass_ptr = (unsigned long long)result;
          unsigned long long klass_sz  = result->size() * wordSize;
