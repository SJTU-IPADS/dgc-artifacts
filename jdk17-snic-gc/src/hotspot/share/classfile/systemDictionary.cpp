...
InstanceKlass* SystemDictionary::resolve_from_stream(ClassFileStream* st,
                                                     ...
                                                     const ClassLoadInfo& cl_info,
                                                     TRAPS) {
  InstanceKlass* ret = NULL;
  if (cl_info.is_hidden()) {
    ret = resolve_hidden_class_from_stream(st, class_name, class_loader, cl_info, CHECK_NULL);
  } else {
    ret = resolve_class_from_stream(st, class_name, class_loader, cl_info, CHECK_NULL);
  }
  //log_dev_debug(gc)("DGC LOG: Calling resolve_from_stream and get instanceKlass name=%s, ptr=%p", class_name->as_klass_external_name(), ret);
  // add logic here to send klass

  if(SnicGCHost){
    /*start @lyh*/
    if (ret != NULL) {
      if (!SnicGCShareMemEnabled) {
        // Under SnicGCRDMABatchFetchKlass, the update_ccs_hwm call is moved
        // to InstanceKlass::link_class_impl so the client only ever sees a
        // Klass once its _vptr/vtable/itable are populated. Arrays skip
        // linking and still hook at allocation (see obj/typeArrayKlass).
        if (!SnicGCRDMABatchFetchKlass) {
          auto k = ret;
          unsigned long long klass_ptr = (unsigned long long)ret;
          unsigned long long klass_sz  = ret->size() * wordSize;
          char buffer[2 * sizeof(int) + 2 * sizeof(unsigned long long)];
          unsigned long long *message = (unsigned long long *)(buffer + 2 * sizeof(int));
          message[0] = klass_ptr;
          message[1] = klass_sz;
          Universe::send_rpc(6, 2 * sizeof(int) + 2 * sizeof(unsigned long long), buffer);
        }
      }
    }
    /*end @lyh*/
...
InstanceKlass* SystemDictionary::load_instance_class_impl(Symbol* class_name, Handle class_loader, TRAPS) {
  ...
  if (class_loader.is_null()) {
    ...
    //log_dev_debug(gc)("DGC LOG: call load_instance_class_impl with class_name=%s", class_name->as_C_string());
    ResourceMark rm(THREAD);
    ...
    if ((obj != NULL) && !(java_lang_Class::is_primitive(obj))) {
      ...
      if (class_name == k->name()) {
        if(SnicGCHost){
          /*start @lyh*/
          if (!SnicGCShareMemEnabled) {
            // SnicGCRDMABatchFetchKlass: hwm is updated at post-linking
            // (InstanceKlass::link_class_impl), not here. See the comment
            // on the resolve_from_stream site for rationale.
            if (!SnicGCRDMABatchFetchKlass) {
              unsigned long long klass_ptr = (unsigned long long)k;
              unsigned long long klass_sz  = k->size() * wordSize;
              char buffer[2 * sizeof(int) + 2 * sizeof(unsigned long long)];
              unsigned long long *message = (unsigned long long *)(buffer + 2 * sizeof(int));
              message[0] = klass_ptr;
              message[1] = klass_sz;
