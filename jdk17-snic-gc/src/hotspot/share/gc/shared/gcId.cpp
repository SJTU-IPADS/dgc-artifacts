...
#include "gc/snicgc/snicCoordinator.hpp"
...
GCIdMark::GCIdMark() : _previous_gc_id(currentNamedthread()->gc_id()) {
  ...
  //   if (currentNamedthread()->gc_id() != (unsigned int)(-1)) {
  //     if (SnicGCShareMemEnabled) {
  //       Atomic::store(&(Universe::get_hostShareGlobalPacerData()->hosts_global_pacer_data[hostId].client_gc_id), (unsigned long long)(currentNamedthread()->gc_id()));
  //     } else {
  //       Atomic::store(&(Universe::get_hostPrivateGlobalPacerData()->client_gc_id), (unsigned long long)(currentNamedthread()->gc_id()));
  //     }
  //   }
  // }
}
...
GCIdMark::GCIdMark(uint gc_id) : _previous_gc_id(currentNamedthread()->gc_id()) {
  ...
  // if(SnicGCCoorHeuristic){
  //   int hostId = Universe::get_CoorHostId();
  //   if (gc_id != (unsigned int)(-1)) {
  //     if (SnicGCShareMemEnabled) {
  //       Atomic::store(&(Universe::get_hostShareGlobalPacerData()->hosts_global_pacer_data[hostId].client_gc_id), (unsigned long long)(gc_id));
  //     } else {
  //       Atomic::store(&(Universe::get_hostPrivateGlobalPacerData()->client_gc_id), (unsigned long long)(gc_id));
  //     }
  //   }
  // }
}
