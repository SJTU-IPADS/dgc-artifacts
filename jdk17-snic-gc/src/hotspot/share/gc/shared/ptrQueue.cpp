...
bool PtrQueueSet::try_enqueue(PtrQueue& queue, void* value) {
  if (value == (void*)((unsigned long long)(-1))) {
    log_debug(gc)("DGC LOG: invalid try enqueue %p,index:%lu", value, queue.index());
  }
  size_t index = queue.index();
  ...
  // TEST ONLY
  if (queue.index() > buffer_size()) {
    log_debug(gc)("DGC LOG: invalid queue index after enqueue: %zu, buffer_size:%zu, update index:%zu", queue.index(), buffer_size(), index);
  }
  return true;
}
