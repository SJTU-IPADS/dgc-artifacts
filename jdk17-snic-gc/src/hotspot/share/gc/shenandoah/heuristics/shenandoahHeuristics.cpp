...
void ShenandoahHeuristics::choose_collection_set(ShenandoahCollectionSet* collection_set) {
  ...
  for (size_t i = 0; i < num_regions; i++) {
    ...
    //log_dev_trace(gc)("DGC LOG: region %d status: %d", i, region->state());
    ...
    if (region->is_empty()) {
      ...
      if (!region->has_live()) {
        //log_dev_trace(gc)("DGC LOG: region all clean");
        // We can recycle it right away and put it in the free set.
        immediate_regions++;
        immediate_garbage += garbage;
        region->make_trash_immediate();
      } else {
        //log_dev_trace(gc)("DGC LOG: region in cset");
...
void ShenandoahHeuristics::record_success_concurrent() {
  ...
  // Skip adding DGC cycle timings to _gc_time_history — DGC cycles involve
  // RDMA round-trips and client-side marking, and have very different
  // wall-clock duration than local fallback/baseline marking. Mixing them
  // inflates avg_cycle_time (mean + 2*stddev) and makes the adaptive
  // predictive trigger fire too eagerly. In non-coor mode the flag is always
  // false so behaviour is unchanged.
  if (!Universe::was_last_cycle_dgc()) {
