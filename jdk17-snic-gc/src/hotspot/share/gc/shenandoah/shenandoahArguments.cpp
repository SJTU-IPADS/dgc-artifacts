...
void ShenandoahArguments::initialize() {
  ...
  // SnicGC host JVMs cannot use the Shenandoah pacer: concurrent marking is
  // offloaded to a remote SNIC client, so the pacer's "wait for GC" logic
  // stalls mutators on a cycle that is not running locally and shows up as
  // long allocation pauses on the host. Force the pacer off whenever
  // SnicGCHost is enabled, regardless of how -XX:[+-]ShenandoahPacing was
  // (or wasn't) supplied on the command line, so users do not have to
  // remember to combine the two flags by hand.
  if (SnicGCHost && ShenandoahPacing) {
    if (!FLAG_IS_DEFAULT(ShenandoahPacing)) {
      log_warning(gc)("Disabling ShenandoahPacing because SnicGCHost is enabled");
    }
