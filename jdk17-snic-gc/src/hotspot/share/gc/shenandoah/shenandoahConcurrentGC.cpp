...
bool ShenandoahConcurrentGC::collect(GCCause::Cause cause) {
...

  {
    ...
    //log_dev_debug(gc)("DGC LOG: do entry_mark_roots");
    /*start @lyh*/
    // TEST ONLY: sleep for 1 sec before conc mark starts
    if (FakeSATBTime > 0) {
      os::naked_short_sleep(FakeSATBTime);
    }
    /*end @lyh*/
    entry_mark_roots();
  ...
  if(SnicGCCoorHeuristic){
    Universe::record_nonmarking_time_start();
  ...
  /*start @lyh*/
  if (SnicGCCoorHeuristic && !ShenandoahHeap::heap()->cancelled_gc()) {
    Universe::record_nonmarking_time_end_and_report();
  }
  /*end @lyh*/
...
/*start @lyh*/
void ShenandoahConcurrentGC::vmop_entry_snic_final_mark() {
  ShenandoahHeap *const heap = ShenandoahHeap::heap();
  TraceCollectorStats tcs(heap->monitoring_support()->stw_collection_counters());
  ShenandoahTimingsTracker timing(ShenandoahPhaseTimings::snic_final_mark);
  heap->try_inject_alloc_failure();
  VM_SnicFinalMark op(this);
  VMThread::execute(&op);
}
/*end @lyh*/
...
void ShenandoahConcurrentGC::vmop_entry_final_mark() {
  ...
  Universe::start_final_mark();
  VMThread::execute(&op); // jump to entry_final_mark under safepoint
  Universe::finish_final_mark();
}
...
/*start @lyh*/
void ShenandoahConcurrentGC::entry_snic_final_mark() {
  const char* msg = snic_final_mark_event_message();
  ShenandoahPausePhase gc_phase(msg, ShenandoahPhaseTimings::snic_final_mark);
  EventMark em("%s", msg);
  ShenandoahWorkerScope scope(ShenandoahHeap::heap()->workers(),
                              1,
                              "snic final marking");
...
/*start @lyh*/
void ShenandoahConcurrentGC::op_snic_final_mark() {
  _mark.snic_final_mark();
}
/*end @lyh*/
...
void ShenandoahConcurrentGC::op_final_mark() {
  ...
  if (!heap->cancelled_gc()) {
    ...
    if (!heap->collection_set()->is_empty()) {
      ...
      //log_dev_debug(gc)("DGC LOG: flag of forwarded pointer is set!!");
...
/*start @lyh*/
const char* ShenandoahConcurrentGC::snic_final_mark_event_message() const {
  ShenandoahHeap *const heap = ShenandoahHeap::heap();
  assert(!heap->has_forwarded_objects(), "Should not have forwarded objects here");
  if (heap->unload_classes()) {
    return "Pause Snic Final Mark (unload classes)";
  } else {
    return "Pause Snic Final Mark";
  }
}
/*end @lyh*/
