...
void ShenandoahControlThread::run_service() {
  ...
  /*start @lyh*/
  double global_pacer_last_report_time_ms = 0;
  /*end @lyh*/
  ...
  while (!in_graceful_shutdown() && !should_terminate()) {
    ...
    if (alloc_failure_pending) {
      ...
      if (SnicGCCoorHeuristic) {
        Universe::coor_set_host_GC_state(Universe::HostGCState::Compacting);
      }
      ...
      /*start @lyh*/
      if (SnicGCHost && SnicGCCoorHeuristic) {
      ...
      if (ShenandoahImplicitGCInvokesConcurrent) {
        if(SnicGCCoorHeuristic){
          if (SnicGCShareMemEnabled) {
            ...
            Universe::finish_global_pacer();
            Universe::finish_global_pacer_Client_Occupied();
          } else {
      ...
      if (heuristics->should_start_gc()) {
        if(SnicGCCoorHeuristic){
          ...
          if (SnicGCShareMemEnabled) {
            Atomic::store(&(Universe::get_hostShareGlobalPacerData()->hosts_global_pacer_data[Universe::get_CoorHostId()].startPacer), (unsigned long long)0);
            Atomic::store(&(Universe::get_hostShareGlobalPacerData()->hosts_global_pacer_data[Universe::get_CoorHostId()].nextGCTime), next_gc_time);
            Universe::finish_global_pacer();
            Universe::finish_global_pacer_Client_Occupied();
          } else {
            Atomic::store(&(Universe::get_hostPrivateGlobalPacerData()->startPacer), (unsigned long long)0);
          ...
          log_debug(gc, ergo)("DGC LOG: set nextGCTime should_start_gc=true: %llu", next_gc_time);
        }
        mode = default_mode;
        cause = default_cause;
      }
    ...
    if (gc_requested) {
      ...
      // if (SnicGCRDMAPrefetchEnabled && !SnicGCShareMemEnabled && Universe::get_start_rdma_prefetch() && !Universe::get_during_rdma_prefetch()) {
      //   log_debug(gc)("DGC LOG: start to perform rdma prefetch");
      //   Universe::set_during_rdma_prefetch(true);
      //   // should perform a prefetch logic here.
      //   // send rpc 8 (or rpc 3) to client first.
      ...
      //   for (size_t i = heap->num_regions(); i > 0; i--) {
      //     ShenandoahHeapRegion *r = heap->get_region(i - 1);
      ...
      //       message[top * 5 + 2] = (unsigned long long)(heap->marking_context()->top_at_mark_start(r));
      ...
      //     }
      //   }
      //   message[top * 5] = (unsigned long long)(1);
      //   if (SnicConcCopyRegion) {
      //     Universe::send_rpc(8, 2 * sizeof(int) + (top * 5 + 1) * sizeof(unsigned long long), buffer);
      //     Universe::recv_int_ack();
      //   } else {
      //     Universe::send_rpc(3, 2 * sizeof(int) + (top * 5 + 1) * sizeof(unsigned long long), buffer);
      //   }
      //   // then send rpc 11 to tell client to start prefetch heap regions.
      //   Universe::send_rpc(11, 2 * sizeof(int), buffer);
      //   Universe::recv_int_ack();
      //   delete (buffer);
      //   continue;
      // }
      ...
      if (ShenandoahPacing && (allocs_seen > 0)) {
        ...
        auto time_diff_ms = 0;
        if (global_pacer_last_report_time_ms != 0) {
          time_diff_ms = cur_time_ms - global_pacer_last_report_time_ms;
          global_pacer_last_report_time_ms = cur_time_ms;
        }
...
bool ShenandoahControlThread::check_global_pacer(){
  return Atomic::load(&(Universe::get_hostShareGlobalPacerData()->hosts_global_pacer_data[Universe::get_CoorHostId()].startPacer));
}

void ShenandoahControlThread::global_pacer_check_mark_progress(){
  unsigned long long budget_tmp = Atomic::load(&(Universe::get_hostShareGlobalPacerData()->hosts_global_pacer_data[Universe::get_CoorHostId()].budgetIncreasing));
  if(budget_tmp > 0){
    Atomic::add(&(Universe::get_hostShareGlobalPacerData()->hosts_global_pacer_data[Universe::get_CoorHostId()].budgetIncreasing), -1 * budget_tmp);
    ShenandoahHeap::heap()->pacer()->report_global_pacer_mark(budget_tmp);
  }
  return;
}
