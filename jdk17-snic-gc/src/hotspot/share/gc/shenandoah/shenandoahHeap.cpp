...
jint ShenandoahHeap::initialize() {
  ...
  if (SnicGCHost && SnicGCShareMemEnabled) {
    ...
    unsigned long long* message = (unsigned long long*)(rpc_buffer + 2 * sizeof(int));
    message[0] = (unsigned long long) (heap_rs.base());
    message[1] = (unsigned long long) (heap_rs.size());
    message[2] = (unsigned long long) (0);
    message[3] = (unsigned long long) (heap_rs.size() / _num_regions);
    log_debug(gc)("DGC LOG: send rpc 1 to share mem client,base:%p,size:%lu,heapRegionSize:%lu", heap_rs.base(), heap_rs.size(), heap_rs.size() / _num_regions);
    // // TEST ONLY
    // unsigned long long* heap_data = (unsigned long long*) (heap_rs.base() + heap_rs.size());
    // heap_data -= 1;
    // *(heap_data) = 654321ULL;
    // log_debug(gc)("DGC LOG: test heap data at %p, %llu", heap_data, *(heap_data));
    Universe::send_rpc(1, sizeof(int) * 2 + sizeof(unsigned long long) * 4, rpc_buffer);
  }
  /*end @lyh*/
  ...
  if (!_heap_region_special) {
    ...
    if(SnicGCHost && !SnicGCShareMemEnabled) {
      /*start @lyh*/
      log_debug(gc)("DGC LOG: init regist mem region for heap:%p,%p", sh_rs.base(), sh_rs.base() + _initial_size);
      /*end @lyh*/

      size_t first_helf_size = (_initial_size / 2) & ~(32 * 1024 * 1024 - 1);
      if (first_helf_size == 0) {
        first_helf_size = _initial_size / 2;  // small heap: skip 32MB alignment
      }
      size_t second_helf_size = _initial_size - first_helf_size;
      Universe::add_mr(sh_rs.base(), first_helf_size, 0);
      Universe::add_mr_nic2(sh_rs.base() + first_helf_size, second_helf_size, 999);

      long long* ptr1 = (long long*)sh_rs.base();
      long long* ptr2 = (long long*)(sh_rs.base() + first_helf_size);
      *ptr1 = 654321ULL;
      *ptr2 = 754321ULL;
      log_debug(gc)("DGC LOG: put random data to heap region");
    }
  }
  ...
  guarantee(((_bitmap_bytes_per_slice) % bitmap_page_size) == 0,
            ...
            _bitmap_bytes_per_slice, bitmap_page_size);
  /*start @lyh*/
  ReservedSpace bitmap;
  if (!(SnicGCHost && SnicGCShareMemEnabled)) {
    bitmap = ReservedSpace(_bitmap_size, bitmap_page_size);
    ...
    sprintf(curShareBitmapPath, "%s_%d", SnicShmBitmapPath, Universe::get_SnicHostId());
    int bitmapFD = shm_open(curShareBitmapPath, O_RDWR | O_CREAT, 0666);
    if(bitmapFD < 0) {
      log_error(gc)("DGC LOG: create share mem file failed, path: %s", curShareBitmapPath);
      exit(0);
    }
    else{
      log_debug(gc)("DGC LOG: create share mem file success, path: %s, fd: %d", curShareBitmapPath, bitmapFD);
    }
    struct stat shm_stat;
    fstat(bitmapFD, &shm_stat);
    auto fileSize = shm_stat.st_size;
    bitmap = ReservedSpace(_bitmap_size, bitmap_page_size);
    bitmap.remap_reserved_space_with_backup(curShareBitmapPath, bitmapFD);
    _bitmap_region = MemRegion((HeapWord*) bitmap.base(), bitmap.size() / HeapWordSize);
    _bitmap_region_special = bitmap.special();
    char* rpc_buffer = new char[1024];
    unsigned long long* message = (unsigned long long*)(rpc_buffer + 2 * sizeof(int));
    message[0] = (unsigned long long) (bitmap.base());
    message[1] = (unsigned long long) (bitmap.size());
    message[2] = (unsigned long long) (fileSize);
    log_debug(gc)("DGC LOG: send rpc 2 to share mem client,cur file size:%ld", fileSize);
    Universe::send_rpc(2, sizeof(int) * 2 + sizeof(unsigned long long) * 3, rpc_buffer);
    close(bitmapFD);
  }
  ...
  if (!_bitmap_region_special) {
    if (!(SnicGCHost && SnicGCShareMemEnabled)) {
      log_debug(gc)("DGC LOG: commit bitmap memory %p, %zu", _bitmap_region.start(), bitmap_init_commit);
      os::commit_memory_or_exit((char *) _bitmap_region.start(), bitmap_init_commit, bitmap_page_size, false, "Cannot commit bitmap memory");
    }

    if(SnicGCHost){
      if (!SnicGCShareMemEnabled) {
        log_debug(gc)("DGC LOG: init regist mem region for bitmap");
        Universe::add_mr(_bitmap_region.start(), bitmap_init_commit, 1);
      }
    }
  }
  /*end @lyh*/
  // TODO: make liveness buffer a RDMA region too
  ...
  if(SnicGCHost){
    if (!SnicGCShareMemEnabled) {
      log_debug(gc)("DGC LOG: init regist mem region for liveness");
      _remote_liveness_cache = NEW_C_HEAP_ARRAY(uint64_t, _num_regions, mtGC);
      Universe::add_mr(_remote_liveness_cache, _num_regions * sizeof(uint64_t), 2);
      /*start @lyh*/
      _snic_gc_roots_buffer = NEW_C_HEAP_ARRAY(char, MAX_RPC_BUFFER_SIZE, mtGC);
      Universe::add_mr(_snic_gc_roots_buffer, MAX_RPC_BUFFER_SIZE, 3);
      log_debug(gc)("DGC LOG: host side gc roots buffer:%p", _snic_gc_roots_buffer);
      _snic_gc_roots_copy_finish_flag = NEW_C_HEAP_ARRAY(int, 1, mtGC);
      Universe::add_mr(_snic_gc_roots_copy_finish_flag, sizeof(int), 4);
      _force_gc_by_dpu_client = NEW_C_HEAP_ARRAY(int, 1, mtGC);
      _force_gc_by_dpu_client[0] = 0;
      _rdma_prefetch_finish_flag = NEW_C_HEAP_ARRAY(int, 1, mtGC);
      _rdma_prefetch_finish_flag[0] = 0;
      _rdma_force_gc_data = new RDMAForceGCData();
      _prefetch_tmp_rdma_force_gc_data = new RDMAForceGCData();
      _rdma_force_gc_data->forceGCId = 0;
      _rdma_force_gc_data->forceGCTriggerTimestamp = 0;
      _rdma_force_gc_data->forceGCType = (unsigned long long) (Universe::ForceGCTypes::EMPTY);
      _rdma_force_gc_data->forceGCCCMT = 0;
      Universe::set_estimated_rdma_copy_time(SnicGCEstimatedRDMACopyTime);
      log_debug(gc)("DGC LOG: host side force gc by dpu client:%p, rdma force gc data:%p, rdma prefetch finish flag:%p", _force_gc_by_dpu_client, _rdma_force_gc_data, _rdma_prefetch_finish_flag);
      if (!SnicGCCoorHeuristic) {
        Universe::add_mr(_force_gc_by_dpu_client, sizeof(int), 5);
      } else {
        Universe::add_mr_for_coor(_rdma_force_gc_data, sizeof(RDMAForceGCData), 5 + SnicGCCoorClientId * 200);
      }
      if (SnicGCCoorHeuristic) {
        GlobalPacerDataPerHost* hostPrivateGlobalPacerData = new GlobalPacerDataPerHost();
        Universe::set_hostPrivateGlobalPacerData(hostPrivateGlobalPacerData);
        Universe::add_mr_for_coor(Universe::get_hostPrivateGlobalPacerData(), sizeof(GlobalPacerDataPerHost), 6 + SnicGCCoorClientId * 200);
        log_debug(gc)("DGC LOG: host side global pacer data:%p, host id for coor:%d", Universe::get_hostPrivateGlobalPacerData(), SnicGCCoorClientId);
      }
      Universe::add_mr(_rdma_prefetch_finish_flag, sizeof(int), 7);
    } else {
      log_debug(gc)("DGC LOG: send rpc for liveness share mem");
      char curShareLivenessPath[1024];
      // strcpy(curShareLivenessPath, "/share_liveness");
      sprintf(curShareLivenessPath, "%s_%d", SnicShmLivenessPath, Universe::get_SnicHostId());
      int livenessFD = shm_open(curShareLivenessPath, O_RDWR | O_CREAT, 0666);
      if(livenessFD < 0) {
        log_error(gc)("DGC LOG: create share mem file failed, path: %s", curShareLivenessPath);
        exit(0);
      }
      else{
        log_debug(gc)("DGC LOG: create share mem file success, path: %s, fd: %d", curShareLivenessPath, livenessFD);
      }
      struct stat shm_stat_1;
      fstat(livenessFD, &shm_stat_1);
      auto liveCacheFileSize = shm_stat_1.st_size;
      _remote_liveness_cache = (uint64_t*) Universe::hostMmap(nullptr, _num_regions * sizeof(uint64_t), (off_t) (liveCacheFileSize), livenessFD, 3);
      // update file size.
      auto newFileSize1 = (size_t) (liveCacheFileSize) + _num_regions * sizeof(uint64_t);
      auto singlePageSize1 = (size_t) os::vm_page_size();
      if (newFileSize1 % singlePageSize1 != 0) {
        newFileSize1 = newFileSize1 - newFileSize1 % singlePageSize1 + singlePageSize1;
      }
      auto trunc_res_1 = ftruncate(livenessFD, (off_t) newFileSize1);
      if (trunc_res_1 != 0) {
        log_error(gc)("DGC LOG: truncate share mem live cache file failed, %d", trunc_res_1);
        exit(0);
      }
      char* rpc_buffer = new char[1024];
      unsigned long long* message = (unsigned long long*)(rpc_buffer + 2 * sizeof(int));
      message[0] = (unsigned long long) _remote_liveness_cache;
      message[1] = (unsigned long long) _num_regions * sizeof(uint64_t);
      message[2] = (unsigned long long) (liveCacheFileSize);
      log_debug(gc)("DGC LOG: send rpc 3 to share mem client,cur file size:%ld", liveCacheFileSize);
      Universe::send_rpc(3, sizeof(int) * 2 + sizeof(unsigned long long) * 3, rpc_buffer);
      close(livenessFD);
      // 2024-12-31: lyh adds shm logic for _regions_tams_info here.
      char curShareRegionTamsPath[1024];
      // strcpy(curShareRegionTamsPath, "/share_region_tams");
      sprintf(curShareRegionTamsPath, "%s_%d", SnicShmRegionTamsPath, Universe::get_SnicHostId());
      int tamsInfoFD = shm_open(curShareRegionTamsPath, O_RDWR | O_CREAT, 0666);
      if(tamsInfoFD < 0) {
        log_error(gc)("DGC LOG: create share mem file failed, path: %s", curShareRegionTamsPath);
        exit(0);
      }
      else{
        log_debug(gc)("DGC LOG: create share mem file success, path: %s, fd: %d", curShareRegionTamsPath, tamsInfoFD);
      }

      unsigned long long tams_share_len = _num_regions * sizeof(unsigned long long);
      unsigned long long tams_share_offset = 0;
      _regions_tams_info = (unsigned long long*) (Universe::hostMmap(nullptr, tams_share_len, tams_share_offset, tamsInfoFD, 4));
      // update file size.
      auto newFileSize2 = tams_share_len;
      if (newFileSize2 % os::vm_page_size() != 0) {
        newFileSize2 = newFileSize2 + os::vm_page_size() - newFileSize2 % os::vm_page_size();
      }
      Universe::ftruncateFile(tamsInfoFD, (off_t)(newFileSize2));
      // then send rpc to client to create shm for _regions_tams_info.
      message[0] = (unsigned long long) _regions_tams_info;
      message[1] = (unsigned long long) tams_share_len;
      message[2] = (unsigned long long) tams_share_offset;
      log_debug(gc)("DGC LOG: send rpc 4 to share mem client");
      Universe::send_rpc(4, 2 * sizeof(int) + 3 * sizeof(unsigned long long), rpc_buffer);
      close(tamsInfoFD);
    }
    /*end @lyh*/
  }
  ...
  if (SnicGCHost && !SnicGCShareMemEnabled) {
    ...
    message[0] = (unsigned long long) (heap_rs.size() / _num_regions);
    log_debug(gc)("DGC LOG: send RPC 1 to copyRegion snic client");
    Universe::send_rpc(1, sizeof(int) * 2 + sizeof(unsigned long long), buffer);
...
private:
  void do_work(uint worker_id) {
    ...
    int total_handle_obj_size = 0;
    int handle_region_count = 0;
    int total_live_size = 0;
    while ((r =_cs->claim_next()) != NULL) {
      assert(r->has_live(), "Region " SIZE_FORMAT " should have been reclaimed early", r->index());
      int handle_obj_size = _sh->marked_object_iterate(r, &cl);
      total_handle_obj_size += handle_obj_size;
      handle_region_count++;
      if (ShenandoahPacing) {
        _sh->pacer()->report_evac(r->used() >> LogHeapWordSize);
      }
      total_live_size += r->get_live_data_bytes();
      if (_sh->check_cancelled_gc_and_yield(_concurrent)) {
        break;
      }
    }
    log_info(gc)("worker %u total handle obj size in bytes:%d, handle region count:%d, total live size:%d", worker_id, total_handle_obj_size, handle_region_count, total_live_size);
  }
};
  ...
  void heap_region_do(ShenandoahHeapRegion* r) {
    if (r->is_active()) {
      ...
      if (top > tams) {
        ...
        log_debug(gc)("DGC LOG: increase_live_data_alloc_words: %lu", pointer_delta(top, tams));
      }
...
void ShenandoahHeap::prepare_regions_and_collection_set(bool concurrent) {
...

  {
    ...
    _collection_set->print_on(&ss);  // 将输出重定向到字符串流
    //log_dev_debug(gc)("DGC LOG: get _collection_set content: %s", ss.as_string());
...
void ShenandoahHeap::flush_liveness_cache(uint worker_id) {
  ...
  for (uint i = 0; i < num_regions(); i++) {
    ...
    if (live > 0) {
      ShenandoahHeapRegion* r = get_region(i);
      //log_dev_trace(gc)("LHG LOG: ShenandoahHeap::flush_liveness_cache of region %d", r->index());
...
void ShenandoahHeap::reset_rdma_force_gc_data() {
  _rdma_force_gc_data->forceGCType = (unsigned long long) Universe::ForceGCTypes::EMPTY;
  _rdma_force_gc_data->forceGCId = 0;
  _rdma_force_gc_data->forceGCTriggerTimestamp = 0;
  _rdma_force_gc_data->forceGCCCMT = 0;
...
void ShenandoahHeap::reset_prefetch_tmp_rdma_force_gc_data() {
  _prefetch_tmp_rdma_force_gc_data->forceGCType = (unsigned long long) Universe::ForceGCTypes::EMPTY;
  _prefetch_tmp_rdma_force_gc_data->forceGCId = 0;
  _prefetch_tmp_rdma_force_gc_data->forceGCTriggerTimestamp = 0;
  _prefetch_tmp_rdma_force_gc_data->forceGCCCMT = 0;
}

bool ShenandoahHeap::handle_rdma_dgc_force_gc() {
  auto received_force_gc_type = _rdma_force_gc_data->forceGCType;
  auto received_force_gc_ccmt = _rdma_force_gc_data->forceGCCCMT;
  auto expected_force_gc_start_ts = _rdma_force_gc_data->forceGCTriggerTimestamp;
  auto received_force_gc_id = _rdma_force_gc_data->forceGCId;
  if (received_force_gc_type == (unsigned long long) (Universe::ForceGCTypes::FallBack) && !ShenandoahVerify) {
    log_debug(gc)("DGC LOG: host %d force gc by coordinator, FallBack, cur ts:%llu, force gc ts:%llu", Universe::get_CoorHostId(), (unsigned long long)os::javaTimeMillis(), expected_force_gc_start_ts);
    ...
    // reset_rdma_force_gc_data();
    workers()->update_active_workers(4);
    ConcGCThreads = ShenandoahHeap::heap()->workers()->active_workers();
    return true;
  } else {
    auto current_ts = (unsigned long long) os::javaTimeMillis();
    log_debug(gc)("DGC LOG: host %d force gc by coordinator, normal DGC, cur ts:%llu, force gc ts:%llu", Universe::get_CoorHostId(), current_ts, expected_force_gc_start_ts);
