...
public:
  ...
  WorkGang* snic_gc_workers() const;
...
public:
  ...
  template<class T>
  inline int marked_object_iterate(ShenandoahHeapRegion* region, T* cl);

  template<class T>
  inline int marked_object_iterate(ShenandoahHeapRegion* region, T* cl, HeapWord* limit);
...
public:
  ...
  bool handle_rdma_dgc_force_gc();
  void reset_rdma_force_gc_data();
  void reset_prefetch_tmp_rdma_force_gc_data();
...
//
public:
  uint64_t* _remote_liveness_cache;
  /*start @lyh*/
  char* _snic_gc_roots_buffer;
  int* _snic_gc_roots_copy_finish_flag;
  unsigned long long* _regions_tams_info;
  int* _force_gc_by_dpu_client;
  int* _rdma_prefetch_finish_flag;
  RDMAForceGCData* _rdma_force_gc_data;
  RDMAForceGCData* _prefetch_tmp_rdma_force_gc_data;
  /*end @lyh*/
};
