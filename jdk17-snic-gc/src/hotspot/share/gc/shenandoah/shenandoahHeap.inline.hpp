...
/*start @lyh*/
inline WorkGang* ShenandoahHeap::snic_gc_workers() const {
  return NULL;
}
/*end @lyh*/
...
inline void ShenandoahHeap::update_with_forwarded(T* p) {
  ...
  if (!CompressedOops::is_null(o)) {
    ...
    if (in_collection_set(obj)) {
      ...
      // log_dev_debug(gc)("DGC LOG: update forwarded pointer");
...
inline oop ShenandoahHeap::evacuate_object(oop p, Thread* thread) {
  //log_dev_trace(gc)("DGC LOG: evacuate_object: %p", p);
...
template<class T>
inline int ShenandoahHeap::marked_object_iterate(ShenandoahHeapRegion* region, T* cl) {
  return marked_object_iterate(region, cl, region->top());
}

template<class T>
inline int ShenandoahHeap::marked_object_iterate(ShenandoahHeapRegion* region, T* cl, HeapWord* limit) {
  assert(! region->is_humongous_continuation(), "no humongous continuation regions here");
  int handle_obj_size = 0;
  
  //log_dev_debug(gc)("DGC LOG: try collect region: %p", region);
  ...
  if (dist > 0) {
    ...
    while (cb < limit_bitmap) {
      ...
      //log_dev_trace(gc)("DGC LOG: evacuate obj %p", obj);
      assert(oopDesc::is_oop(obj), "sanity");
      assert(ctx->is_marked(obj), "object expected to be marked");
      cl->do_object(obj);
      cb += skip_bitmap_delta;
  ...
  while (cs < limit) {
    ...
    // DIAG: log the object header before dereferencing the klass, so if
    // obj->size() crashes we know exactly which cs, where in [tams, limit),
    // and what the object header looks like. Only active for SnicGCHost to
    // keep overhead off the normal path.
    if (SnicGCHost) {
      unsigned long long hdr0 = *(unsigned long long*)cs;
      unsigned long long hdr1 = *((unsigned long long*)cs + 1);
      Klass* k_raw = obj->klass_or_null();
      if (k_raw == nullptr) {
        log_error(gc)("DGC LOG: marked_object_iterate Step 2 CRASH imminent at cs=" PTR_FORMAT
                      " region=[%p..%p) tams=" PTR_FORMAT " limit=" PTR_FORMAT
                      " hdr[0]=0x%llx hdr[1]=0x%llx klass=nullptr",
                      p2i(cs), region->bottom(), region->end(), p2i(tams), p2i(limit),
                      hdr0, hdr1);
      } else {
        log_debug(gc)("DGC LOG: Step 2 cs=" PTR_FORMAT " hdr[0]=0x%llx hdr[1]=0x%llx klass=%p",
                      p2i(cs), hdr0, hdr1, (void*)k_raw);
      }
    }
    assert(oopDesc::is_oop(obj), "sanity");
    assert(ctx->is_marked(obj), "object expected to be marked");
    int size = obj->size();
    handle_obj_size += size;
    cl->do_object(obj);
    cs += size;
  }
  return handle_obj_size;
}
