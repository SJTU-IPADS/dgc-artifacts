...
void ShenandoahHeapRegion::make_regular_allocation() {
  ...
  // std::cout<<"DGC LOG: make_regular_allocation "<<_state<<std::endl;
  switch (_state) {
...
void ShenandoahHeapRegion::set_live_data(size_t s) {
  ...
  //log_dev_trace(gc)("DGC LOG: set liveness data, region idx:%lld, new_live_data:%lld", index(), s >> LogHeapWordSize);
}
...
void ShenandoahHeapRegion::do_commit() {
  ...
  // }
  // std::cout<<"DGC LOG: start regist mem region "<<_index<<std::endl;
  if (!heap->is_heap_region_special()) {
    auto succ = os::commit_memory((char *) bottom(), RegionSizeBytes, false);
    if(succ){
      // modified @lht
      // regist rdma memory region
      if(SnicGCHost){
        log_debug(gc)("DGC LOG: regist mem region ");
        Universe::add_mr((void*)bottom(), (size_t)RegionSizeBytes, _index);
