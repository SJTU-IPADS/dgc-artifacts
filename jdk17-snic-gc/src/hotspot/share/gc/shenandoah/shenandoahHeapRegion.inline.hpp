...
inline void ShenandoahHeapRegion::internal_increase_live_data(size_t s) {
  ...
  auto r_index = index();
  //log_dev_trace(gc)("DGC LOG: update liveness data, region idx:%lu, increase:%lu, new_live_data:%lu", r_index, s, new_live_data);
#ifdef ASSERT
...
inline size_t ShenandoahHeapRegion::garbage() const {
  ...
  auto used_bytes = used();
  //log_dev_trace(gc)("DGC LOG: call ShenandoahHeapRegion::garbage(), get garbage=%lu, used=%lu, live=%lu", result, used_bytes, get_live_data_bytes());
  return result;
}
