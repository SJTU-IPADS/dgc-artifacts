...
inline void ShenandoahMark::count_liveness(ShenandoahLiveData* live_data, oop obj) {
  //log_dev_trace(gc)("LHG LOG: ShenandoahMark::count_liveness, obj addr: %llp, klass name: %s", obj, obj->klass()->name()->as_C_string());
...
/*start @lyh*/
class SnicSATBThreadsClosure : public ThreadClosure {
private:
  SATBMarkQueueSet &_satb_qset;
public:
  SnicSATBThreadsClosure(SATBMarkQueueSet &qset) : _satb_qset(qset) {}
  void do_thread(Thread *thread) {
    _satb_qset.flush_queue(ShenandoahThreadLocalData::satb_mark_queue(thread));
  }
};

class SnicSATBBufferClosure : public SATBBufferClosure {
private:
  // char** _buf;
  int* count;
  size_t _buffer_size;
  int* _rpc_count;
public:
  SnicSATBBufferClosure(int* cnt, size_t buffer_size) : count(cnt), _buffer_size(buffer_size) {}
  void do_buffer(void** buffer, size_t size) {
    // if (size > 1024) {
    //   log_debug(gc)("DGC LOG:SATB large queue size:%lu", size);
    // }
    // unsigned long long* _msg_buf = (unsigned long long*) (*_buf + 2 * sizeof(int));
    // size_t N = (_buffer_size - 2 * sizeof(int)) / sizeof(unsigned long long);
    unsigned long long* _msg_buf = (unsigned long long*) (ShenandoahHeap::heap()->_snic_gc_roots_buffer);
    size_t N = _buffer_size / sizeof(unsigned long long);
    int pos = *(count);
    if (pos + size + 1 >= N) {
      log_debug(gc)("DGC LOG:SATB buffer overflow, pos:%d, size:%lu, buffer_size:%lu", pos, size, _buffer_size);
      exit(0);
    }
    for (size_t i = 0; i < size; ++i) {
      unsigned long long data = ((unsigned long long) buffer[i]);
      // if (data < reinterpret_cast<unsigned long long> (_heap->reserved_region().start()) || data >= reinterpret_cast<unsigned long long> (_heap->reserved_region().end())) {
      //   log_debug(gc)("DGC LOG:out of heap range, data:%llx, heapBase:%p, heapSize:%p", data, _heap->reserved_region().start(), _heap->reserved_region().end());
      //   exit(0);
      // }
      _msg_buf[pos] = data;
      pos++;
    }
    // log_debug(gc)("DGC LOG:SATB buffer pos:%d", pos);
    _msg_buf[pos] = (unsigned long long) (-1);
    pos++;
    *(count) = pos;
  }
};

class SnicShareMemSATBBufferClosure : public SATBBufferClosure {
private:
  unsigned long long* _roots_buffer;
  int* _top;
  int _buf_cnt = 0;
public:
  SnicShareMemSATBBufferClosure(unsigned long long* roots_buffer, int* top) : _roots_buffer(roots_buffer), _top(top) {}
  void do_buffer(void** buffer, size_t size) {
    log_debug(gc)("DGC LOG: buffer %d size: %lu", _buf_cnt, size);
    _buf_cnt++;
    int cur_pos = *(_top);
    for (size_t i = 0; i < size; ++i) {
      _roots_buffer[cur_pos++] = (unsigned long long) buffer[i];
    }
    _roots_buffer[cur_pos] = (unsigned long long) (-1);
    *(_top) = cur_pos + 1;
  }
};

class SnicCollectSATBRootsToTaskQueueClosure : public SATBBufferClosure {
private:
  ShenandoahObjToScanQueue *_queue;
  std::vector<unsigned long long> *_vec;
  std::vector<size_t> *_buf_sizes_vec;
public:
  SnicCollectSATBRootsToTaskQueueClosure(ShenandoahObjToScanQueue *queue, std::vector<unsigned long long> *vec, std::vector<size_t> *buf_sizes_vec) : _queue(queue), _vec(vec), _buf_sizes_vec(buf_sizes_vec) {}
  void do_buffer(void **buffer, size_t size) {
    for (size_t i = 0; i < size; ++i) {
      oop *p = (oop *) &buffer[i];
      // _queue->push(ShenandoahMarkTask(*p, false, false));
      _vec->push_back((unsigned long long) *p);
    }
    _buf_sizes_vec->push_back(size);
  }
};
/*end @lyh*/
...
public:
  ...
  void do_buffer_impl(void **buffer, size_t size) {
    for (size_t i = 0; i < size; ++i) {
      ...
      if (SnicGCHost && buffer[i] != nullptr) {
        extern volatile uint64_t g_satb_drained_by_host;
        Atomic::inc(&g_satb_drained_by_host);
      }
    }
  }
};
...
inline void ShenandoahMark::mark_through_ref(T* p, ShenandoahObjToScanQueue* q, ShenandoahMarkingContext* const mark_context, StringDedup::Requests* const req, bool weak) {
  ...
  // log_dev_debug(gc)("DGC LOG: mark_through_ref from ptr: %p", p);
  if (!CompressedOops::is_null(o)) {
    oop obj = CompressedOops::decode_not_null(o);
    // log_dev_debug(gc)("DGC LOG: mark_through_ref load obj addr: %p", obj);
    shenandoah_assert_not_forwarded(p, obj);
    shenandoah_assert_not_in_cset_except(p, obj, ShenandoahHeap::heap()->cancelled_gc());
    if (mark_context->allocated_after_mark_start(obj)) {
      return;
    }
