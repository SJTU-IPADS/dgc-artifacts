...
ShenandoahGCSession::ShenandoahGCSession(GCCause::Cause cause) :
  ...
  //   // Universe::coor_set_GCstart_host();
...
ShenandoahGCSession::~ShenandoahGCSession() {
  ...
  // dont call coor_set_GCend_host, make sure the not-GC flag is set when necessary matric is calculated, move to AdaptiveHeuristics
  if(SnicGCCoorHeuristic){
    Universe::coor_unset_forceGC();
  }
}
