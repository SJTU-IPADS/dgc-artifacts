...
/*start @lyh*/
class VM_SnicFinalMark : public VM_ShenandoahOperation {
private:
  ShenandoahConcurrentGC *const _gc;
public:
  VM_SnicFinalMark(ShenandoahConcurrentGC *gc) : VM_ShenandoahOperation(), _gc(gc) {};
  VM_Operation::VMOp_Type type() const { return VMOp_SnicFinalMark; }
  const char *name() const { return "Snic Final Mark"; }
  virtual void doit();
};
/*end @lyh*/
