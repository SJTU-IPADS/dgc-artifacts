...
void Metaspace::initialize_class_space(ReservedSpace rs) {
  ...
  if (SnicGCHost) {
    if (!SnicGCShareMemEnabled) {
      ...
      log_debug(gc)("DGC LOG: send rpc 8 to share mem client,space base:%p,size:%lu,backupFileOffset:%lu", rs.base(), rs.size(), rs._backupFileOffset);
...
void Metaspace::global_initialize() {
  ...
  if (using_class_space() && !class_space_is_initialized()) {
    ...
    if (CompressedClassSpaceBaseAddress != 0) {
      ...
      if (rs.is_reserved()) {
        log_info(gc, metaspace)("Sucessfully forced class space address to " PTR_FORMAT, p2i(base));
      } else {
        vm_exit_during_initialization(
    ...
    if (SnicGCHost && SnicGCShareMemEnabled) {
      log_debug(gc)("DGC LOG: rpc_8 case 2");
      char path[1024];
