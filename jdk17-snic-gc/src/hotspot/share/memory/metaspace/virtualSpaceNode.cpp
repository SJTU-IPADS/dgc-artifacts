...
bool VirtualSpaceNode::commit_range(MetaWord* p, size_t word_size) {
  ...
  // Commit...
  // DGC LOG: Note that we move commit before reserve memory in SNIC GC HOST
  if(!SnicGCHost){
    ...
    // log_dev_trace(metaspace)("DGC LOG: skip commit from %p to %p", (void*)p, (char*)p + word_size * BytesPerWord);
  }
...
void VirtualSpaceNode::uncommit_range(MetaWord* p, size_t word_size) {
  ...
  if(!SnicGCHost){
    ...
    // log_dev_trace(metaspace)("DGC LOG: skip uncommit from %p to %p", (void*)p, (char*)p + word_size * BytesPerWord);
  }
                                                ...
                                                SizeCounter* commit_words_counter)
{
  ...
  if (SnicGCHost && SnicGCShareMemEnabled) {
    log_debug(gc)("DGC LOG: rpc_8 case 3");
    if ((unsigned long long) (rs.base()) != 0x800000000) {
      char path[1024];
      ...
      rs.remap_reserved_space_with_backup(path, fd);
    }
  }
  if(SnicGCHost){
    if (!SnicGCShareMemEnabled) {
      rs.commit_memory_and_reg_rdma();
    } else {
      // os::commit_memory_or_exit((char *)rs.base(), rs.size(), -1, "Cannot commit metadata space");
      log_debug(gc)("DGC LOG: send rpc 8 to share mem client");
      char *buffer = new char[1024];
      unsigned long long *message = (unsigned long long *)(buffer + 2 * sizeof(int));
      message[0] = (unsigned long long)rs.base();
      message[1] = rs.size();
      message[2] = rs._backupFileOffset;
      Universe::send_rpc(8, 2 * sizeof(int) + 3 * sizeof(unsigned long long), buffer);
    }
  }
  /*end @lyh*/
