...
void ReservedSpace::commit_memory_and_reg_rdma(){
  log_debug(gc)("DGC LOG: pre commit from %p to %p", (void *)_base, _base + _size);
  os::commit_memory_or_exit((char *)_base, _size, -1,
                            "Cannot commit metadata space");

  int mr_idx = Universe::add_mr((void *)_base, _size);

  log_debug(gc)("DGC LOG: Start sending RPC 5 to server. ");
  int buffer[3];
  buffer[2] = mr_idx;
  Universe::send_rpc(5, 3*sizeof(int), buffer);

  // Wait for client to finish ibv_reg_mr before proceeding. The client's
  // ibv_reg_mr for large Metaspace MRs (up to 4GB) takes ~800ms and blocks
  // its RPC thread. If we don't wait here, the host's first GC may fire
  // before the client is ready, queuing RPC 8 behind ibv_reg_mr and causing
  // a Degen cascade that OOMs small-heap benchmarks (spring, tomcat).
  Universe::recv_int_ack();
  log_debug(gc)("DGC LOG: RPC 5 ack received, client MR registration complete.");
}

/*start @lyh*/
void ReservedSpace::remap_reserved_space_with_backup(const char* backup_file_path, int fd) {
  // Swap a JVM-reserved VA range (heap/bitmap/metaspace VSN/compressed-class)
  // for a SHM-backed mapping so the DGC client can observe it. Caller owns
  // the original `base()` — see metaspace.cpp / shenandoahHeap.cpp / metaspaceShared.cpp.
  auto space_start = base();
  // CDS archive is anchored at a fixed JVM-ergonomic address (0x800000000)
  // and already maps its backing file, so nothing to rewrite.
  if (UseSharedSpaces && (unsigned long long)space_start == 0x800000000) {
    return;
  }
  auto space_size = size();

  // Relocate kernel-picked reservations (marking bitmap, non-class
  // Metaspace VSNs) into the pre-reserved SHM arena so the DGC client
  // can MAP_FIXED-overlay the same address. Addresses above
  // 0x7f0000000000 are kernel-picked (near libjvm/stack) and reliably
  // collide in the client's process. Addresses below that are JVM-
  // ergonomic (CompressedClassSpaceBaseAddress at 0x880000000, heap at
  // 0x500000000, etc) — those are pinned and the oops-encoding/decoding
  // machinery assumes them, so we must NOT relocate.
  char* target = space_start;
  bool relocated = false;
  const unsigned long long KERNEL_PICKED_THRESHOLD = 0x7f0000000000ULL;
  if (Universe::shmArenaBase != nullptr &&
      (unsigned long long) space_start >= KERNEL_PICKED_THRESHOLD) {
    // Preserve the caller's alignment when relocating — Metaspace VSNs
    // need MAX_CHUNK_BYTE_SIZE alignment (16 MB or so), Shenandoah
    // marking bitmap can tolerate page alignment.
    void* arena_slot = Universe::bumpAllocInShmArena(space_size, _alignment);
    if (arena_slot != nullptr) {
      target = (char*) arena_slot;
      relocated = true;
    }
  }

  // Unmap the original JVM-chosen location.
  auto unmap_res = os::unmap_memory(space_start, space_size);
  if (!unmap_res) {
    log_error(gc)("DGC LOG: unmap memory failed, %p, %zu", space_start, space_size);
    exit(0);
  }

  // mmap the backup-file fd at the (possibly relocated) target address.
  struct stat shm_stat;
  fstat(fd, &shm_stat);
  auto fileSize = shm_stat.st_size;
  int fixed = relocated ? MAP_FIXED : 0;
  void* mmap_res = mmap(target, space_size, PROT_READ | PROT_WRITE,
                        MAP_SHARED | fixed, fd, (off_t)(fileSize));
  if (mmap_res == MAP_FAILED || (relocated && mmap_res != target)) {
    log_error(gc)("DGC LOG: remap mmap failed, errno=%d %s, target=%p, old=%p, size=%zu",
                  errno, strerror(errno), target, space_start, space_size);
    exit(0);
  }
  if (!relocated && mmap_res != space_start) {
    log_error(gc)("DGC LOG: remap addr drift: wanted %p got %p (no arena)",
                  space_start, mmap_res);
    exit(0);
  }

  // Update _base so subsequent callers see the relocated address. This
  // is safe because the caller hasn't cached the old base past this point
  // (metaspace/shenandoah read base() after calling remap).
  if (relocated) {
    _base = target;
    log_debug(gc)("DGC LOG: remap relocated %p -> %p, size=%zu", space_start, target, space_size);
  }

  _backupFileOffset = fileSize;
  // update backup file size.
  auto newFileSize = (size_t) (fileSize) + space_size;
  auto singlePageSize = (size_t) os::vm_page_size();
  if (newFileSize % singlePageSize != 0) {
    newFileSize = newFileSize - newFileSize % singlePageSize + singlePageSize;
  }
  auto truncate_result = ftruncate(fd, (off_t) newFileSize);
  if (truncate_result != 0) {
    log_error(gc)("DGC LOG: ftruncate failed, %s", strerror(errno));
    exit(0);
  }
  log_debug(gc)("DGC LOG: remap success: %p, %zu, %zu", target, space_size, fileSize);
}
/*end @lyh*/
...
/*start @lyh*/
ReservedHeapSpace::ReservedHeapSpace() : ReservedSpace() {}
/*end @lyh*/
...
ReservedHeapSpace::ReservedHeapSpace(size_t size, size_t alignment, size_t page_size, const char* heap_allocation_directory) : ReservedSpace() {
  ...
  /*start @lyh*/
  if ((SnicGCHost && SnicGCShareMemEnabled) || NormalGCShmEnabled) {
    log_debug(gc)("DGC LOG: backup fd for heap shared memory:%d", Universe::get_hostShareHeapFD());
    _fd_for_heap = Universe::get_hostShareHeapFD();
  }
  /*end @lyh*/

  if (heap_allocation_directory != NULL && _fd_for_heap == -1) {
  ...
  if (_fd_for_heap != -1 && !(SnicGCHost && SnicGCShareMemEnabled)) {
