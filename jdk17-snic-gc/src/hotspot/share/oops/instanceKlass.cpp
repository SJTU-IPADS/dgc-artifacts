...
bool InstanceKlass::link_class_impl(TRAPS) {
  ...
  // SnicGCRDMABatchFetchKlass: bump the CCS high-water mark only AFTER
  // linking fills in _vptr, vtable, itable, and method dispatch state.
  // Previously this fired at resolve_from_stream return (Klass allocated
  // but not yet linked), so the client could bulk-sync a Klass whose
  // _vptr was still 0 and the DGC marker SEGV'd dereferencing it.
  // Arrays (obj/typeArrayKlass) are fully initialized at allocation time
  // and keep their original at-allocation update_ccs_hwm hook.
  if (SnicGCHost && !SnicGCShareMemEnabled && SnicGCRDMABatchFetchKlass) {
    unsigned long long klass_ptr = (unsigned long long)this;
    unsigned long long klass_sz  = this->size() * wordSize;
    Universe::update_ccs_hwm(klass_ptr + klass_sz);
  }
  return true;
}
...
instanceOop InstanceKlass::allocate_instance(TRAPS) {
  ...
  // if(SnicGCHost){
    // std::cout<<"DGC LOG: try alloc instance, get oop: " << std::hex << (long long) i << std::dec << " klass name is: "<<internal_name()<<std::endl;
    //log_dev_debug(gc)("DGC LOG: try alloc instance, get oop: %llx, size: %llx, klass name is: %s", (intptr_t)i, size, internal_name());
  // }
