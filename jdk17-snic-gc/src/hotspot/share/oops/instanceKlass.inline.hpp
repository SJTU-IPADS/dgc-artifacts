...
ALWAYSINLINE void InstanceKlass::oop_oop_iterate_oop_map(OopMapBlock* map, oop obj, OopClosureType* closure) {
  ...
  //log_dev_trace(gc)("DGC LOG: oop_oop_iterate_oop_map, obj=%p, p=%p, end=%p", obj, p, end);
  for (; p < end; ++p) {
    // std::cout << "DGC LOG: do oop on address: " << p << std::endl;
...
ALWAYSINLINE void InstanceKlass::oop_oop_iterate_oop_map_reverse(OopMapBlock* map, oop obj, OopClosureType* closure) {
  ...
  T*       p     = start + map->count();
  //log_dev_trace(gc)("DGC LOG: oop_oop_iterate_oop_map_reverse, obj=%p, p=%p, end=%p", obj, start, p);
  while (start < p) {
    --p;
    Devirtualizer::do_oop(closure, p);
  }
...
ALWAYSINLINE void InstanceKlass::oop_oop_iterate_oop_map_bounded(OopMapBlock* map, oop obj, OopClosureType* closure, MemRegion mr) {
  T* p   = (T*)obj->obj_field_addr<T>(map->offset());
  T* end = p + map->count();
  //log_dev_trace(gc)("DGC LOG: oop_oop_iterate_oop_map_bounded, obj=%p, p=%p, end=%p", obj, p, end);
...
ALWAYSINLINE void InstanceKlass::oop_oop_iterate_oop_maps(oop obj, OopClosureType* closure) {
  ...
  //log_dev_trace(gc)("DGC LOG: oop_oop_iterate_oop_maps, obj=%p, klass=%p, OopMapBlock map=%p, end_map=%p", obj, this, map, end_map);
  for (; map < end_map; ++map) {
...
ALWAYSINLINE void InstanceKlass::oop_oop_iterate_oop_maps_reverse(oop obj, OopClosureType* closure) {
  ...
  //log_dev_trace(gc)("DGC LOG: oop_oop_iterate_oop_maps_reverse, obj=%p, klass=%p, OopMapBlock map=%p, end_map=%p", obj, this, start_map, map);
  while (start_map < map) {
    --map;
...
ALWAYSINLINE void InstanceKlass::oop_oop_iterate_oop_maps_bounded(oop obj, OopClosureType* closure, MemRegion mr) {
  OopMapBlock* map           = start_of_nonstatic_oop_maps();
  OopMapBlock* const end_map = map + nonstatic_oop_map_count();
  //log_dev_trace(gc)("DGC LOG: oop_oop_iterate_oop_maps_bounded, obj=%p, klass=%p, OopMapBlock map=%p, end_map=%p", obj, this, map, end_map);
  for (;map < end_map; ++map) {
