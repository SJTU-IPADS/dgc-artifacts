...
public:
  ...
  retry:
    ...
    if (res == IOCode::Ok) {
      ...
      // std::cout<<"lht log: unknown err in channel->recv"<<std::endl;
    ...
    for (channel->start(1000000); channel->has_msg(); channel->next(), count += 1) {
      ...
      // try 
      {
        ...
        // try 
        {
          ...
          auto seg_opt = MultiMsg<kMaxMsgSz>::create_from(msg);
          if (!seg_opt.has_value()) {
            RDMA_LOG(2) << "SRpcHandler: MultiMsg::create_from failed, skip";
            continue;
          }
          segmeneted_msg = std::move(seg_opt).value();

          // query the RPC call id
          auto hdr_buf = segmeneted_msg.query_one(0);
          if (!hdr_buf.has_value()) {
            RDMA_LOG(2) << "SRpcHandler: query_one(0) header empty, skip";
            continue;
          }
          auto hdr_opt = ::rdmaio::Marshal::dedump<SRpcHeader>(hdr_buf.value());
          if (!hdr_opt.has_value()) {
            RDMA_LOG(2) << "SRpcHandler: dedump<SRpcHeader> failed, skip";
            continue;
          }
          header = hdr_opt.value();
        ...
        auto param_opt = segmeneted_msg.query_one(1);
        if (!param_opt.has_value()) {
          RDMA_LOG(2) << "SRpcHandler: query_one(1) parameter empty, skip rpc id=" << (int)id;
          continue;
        }
        ByteBuffer parameter = param_opt.value();

        // call the RPC
        ByteBuffer reply = factory.call_one(id, parameter);

        auto coded_opt = MultiMsg<kMaxMsgSz>::create_exact(sizeof(SReplyHeader) +
                                                           reply.size());
        if (!coded_opt.has_value()) {
          RDMA_LOG(2) << "SRpcHandler: create_exact for reply failed, skip rpc id=" << (int)id;
          continue;
        }
        MultiMsg<kMaxMsgSz> coded_reply = std::move(coded_opt).value();
        coded_reply.append(::rdmaio::Marshal::dump<SReplyHeader>(
