  ...
  Result<std::string> wait_ready(const double &timeout_usec,
    ...
    for (uint i = 0; i < retry; ++i) {
      ...
      if (res != IOCode::Ok && res != IOCode::Timeout){
        ...
        // std::cout<<"lht log: error in rpc.call"<<std::endl;
        return res;
      }
      auto res_reply =
          rpc.receive_reply(timeout_usec, true); // receive as hearbeat message
      if (res_reply == IOCode::Ok){
        // std::cout<<"lht log: ok in res_reply"<<std::endl;
        return res_reply;
      }
      if (res_reply == IOCode::Err){
        // std::cout<<"lht log: err in res_reply"<<std::endl;
        return res_reply;
      }
    }
  ...
  Result<std::string> delete_remote_rc(const std::string &name, const u64 &key,
    ...
    if (res_reply == IOCode::Ok) {
      // try 
      {
        ...
        default:
          return ::rdmaio::Err(std::string("unknown error"));
        }
      }
      ...
      if (res_reply == IOCode::Ok) {
        // try 
        {
          ...
          case proto::CallbackStatus::WrongArg:
            ...
            goto ErrCase;
          default:
            // std::cout<<"lht log: get unknown err at cc_rc:dedump"<<std::endl;
    ...
    if (res_reply == IOCode::Ok) {
      ...
      // try 
      {
        ...
        case proto::CallbackStatus::NotFound:
          return NotReady(std::make_pair(std::string("NotReady"), rmem::RegAttr()));
        default:
          return ::rdmaio::Err(
