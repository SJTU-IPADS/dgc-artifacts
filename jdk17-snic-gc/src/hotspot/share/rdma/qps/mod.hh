...
public:
  ...
  /*!
    Batch-poll up to `max` completions from send_cq into `out[]`.
    Returns number of completions fetched (>= 0). Does NOT block;
    returns 0 if the CQ is currently empty.

    Used by hot paths that post many signaled RDMA operations (e.g. DGC
    region copies) and need to drain completions without paying the
    per-CQE round-trip of ibv_poll_cq(cq, 1, ...).
   */
  inline int poll_send_comp_batch(int max, ibv_wc *out) {
    int n = ibv_poll_cq(cq, max, out);
    if (n > 0) out_signaled -= n;
    return n;
  }
