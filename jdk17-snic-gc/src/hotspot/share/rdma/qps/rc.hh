...
public:
  ...
  Result<std::string> send_normal_direct(const ReqDesc &desc,
                                  const ReqPayload &payload) {
    return send_normal_direct(desc, payload, local_mr.value(), remote_mr.value());
  }
  ...
  Result<std::string> send_normal_direct(const ReqDesc &desc,
    ...
    sr.wr.rdma.remote_addr = payload.remote_addr;
    sr.wr.rdma.rkey = remote_mr.key;
