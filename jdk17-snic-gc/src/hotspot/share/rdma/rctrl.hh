...
private:
  ByteBuffer fetch_mr_handler(const ByteBuffer &b) {
    ...
    RDMA_LOG(ERROR) << "fetch mr fails, WrongArg";
    return ::rdmaio::Marshal::dump<proto::MRReply>(
  ...
  WA:
    RDMA_LOG(ERROR) << "delete rc fails, WrongArg";
    return ::rdmaio::Marshal::dump<proto::RCReply>(
        {.status = proto::CallbackStatus::WrongArg});
  }
...
private:
  ...
  ByteBuffer rc_handler(const ByteBuffer &b) {
    RDMA_LOG(INFO) << "Start handle RC connect request";

    auto rc_req_o = ::rdmaio::Marshal::dedump<proto::RCReq>(b);
    if (!rc_req_o){
      RDMA_LOG(ERROR) << "RC connect fails to dedump";
      goto WA;
    }

    {
      ...
      if (!(rc_req.whether_create == static_cast<u8>(1) ||
        ...
        RDMA_LOG(ERROR) << "RC connect fail since whether_create==0";
        goto WA;
      }
      ...
      if (rc_req.whether_create == 1) {
        ...
        if (!nic){
          RDMA_LOG(ERROR) << "RC connect fail, failed to find Nic";
        ...
        if (!rc_status) {
          // clean up
          RDMA_LOG(ERROR) << "RC connect fail, failed to create and register this QP";
          goto WA;
        }
        ...
        if (rc->connect(rc_req.attr) != IOCode::Ok) {
          ...
          RDMA_LOG(ERROR) << "RC connect fail, failed to connect the QP";
          goto WA;
        }
        key = rc_status.value();
      }
  ...
  WA: // wrong arg
    RDMA_LOG(ERROR) << "RC connect fail, return error";
    return ::rdmaio::Marshal::dump<proto::RCReply>(
        {.status = proto::CallbackStatus::WrongArg});
  Err:
    return ::rdmaio::Marshal::dump<proto::RCReply>(
