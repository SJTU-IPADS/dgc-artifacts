...
public:
  RegHandler(const Arc<RMem> &mem, const Arc<RNic> &nic,
    ...
    if (rnic->valid()) {
      ...
      // Clear errno so the failure branch reads a meaningful value.
      // ibv_reg_mr can invoke internal syscalls that leave errno set on a
      // return path that still reports success — the leftover errno has
      // misled prior debugging sessions into chasing phantom ENOSPCs.
      errno = 0;
      ...
      if (!valid()) {
        ...
        RDMA_LOG(4) << "register mr failed at addr: (" << raw_ptr << ","
                    << raw_sz << ") flags=0x" << std::hex << flags.get_value()
                    << std::dec << " errno=" << reg_errno
                    << " (" << std::strerror(reg_errno) << ")";
        exit(0);
      }
    }
    // end class init
