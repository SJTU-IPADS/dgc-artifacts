...
#define RUNTIME_FLAGS(develop,                                              \
  ...
  product(size_t, CompressedClassSpaceSize, 1*G,                            \
          ...
          range(1*M, 3*G)                                                   \
                                                                            \
  product(size_t, CompressedClassSpaceBaseAddress, 0,                       \
  ...
  product(size_t, SharedBaseAddress, LP64_ONLY(32*G)                        \
          ...
          range(0, SIZE_MAX)                                                \
                                                                            \
  product(size_t, SnicShareBaseAddress, LP64_ONLY(32*G)                     \
          NOT_LP64(LINUX_ONLY(2*G) NOT_LINUX(0)),                           \
          "snic Address to allocate shared memory region for class data")   \
          range(0, SIZE_MAX)                                                \
                                                                            \
  ...
  develop(bool, TraceOptimizedUpcallStubs, false,                               \
                "Trace optimized upcall stub generation")                       \
                                                                                \
  product(bool, SnicGCHost , false,                                             \
          "Use this to enable SnicGC on host")                                  \
  product(int, SnicGCHostNum , 1,                                               \
          "Number of hosts in the SnicGC")                                      \
  product(int, FakeSATBTime, 0,                                                 \
          "Max pause time for SnicGC on host")                                  \
                                                                                \
  product(bool, SnicGCClient , false,                                           \
          "Use this to enable SnicGC on dpu side")                              \
                                                                                \
  product(double, SnicAvgMarkTimeAmplifyRate, 1.1,                              \
          "Amplify rate for avg mark time")                                     \
                                                                                \
  product(bool, SnicGCCoordinator , false,                                      \
          "Use this to enable SnicGC coordinator")                              \
                                                                                \
  product(bool, SnicGCRDMAPrefetchEnabled, false,                               \
          "Use this to enable RDMA heap regions prefetch for SnicGC")           \
                                                                                \
  product(bool, SnicGCRDMABatchFetchKlass, false,                               \
          "Batch per-class klass RDMA fetches into periodic bulk transfers. "   \
          "Host tracks a CCS high-water mark; on each force-gc RPC the client " \
          "recreates its CCS QP and issues one RDMA READ covering the delta "   \
          "since the last sync. Works around mlx5 fw 32.42.1000 "               \
          "IBV_WC_LOC_QP_OP_ERR / vendor_err=0x68 on back-to-back small RDMA "  \
          "READs over the same QP (see osdi26-ae KNOWN_ISSUES §1). Enable "     \
          "for tradesoap / tradebeans / spring; leave off for h2/tomcat "       \
          "where the per-class path hasn't been observed to fail.")             \
                                                                                \
  product(int, SnicGCEstimatedRDMACopyTime, 150,                                \
          "Estimated time for RDMA copy time")                                  \
                                                                                \
  product(bool, SnicGCCoorHeuristic, false,                                     \
          "whether use Coor as controller")                                     \
                                                                                \
  product(ccstr, SnicGCCoorSHMPath, "/coor_heuristic",                          \
          "path for Coor as controller")                                        \
                                                                                \
  product(int, SnicGCCoorClientNum, 1,                                          \
          "Number of clients for Coor as controller")                           \
                                                                                \
  product(int, SnicGCCoorClientId, 0,                                           \
          "Id of this client for Coor as controller")                           \
                                                                                \
  product(int, DpuClientMarkerNum, 16,                                          \
          "Number of markers for SnicGC on dpu side")                           \
                                                                                \
  product(int, DpuClientLivenessUpdateThreshold, 50000,                         \
          "Liveness update threshold for SnicGC on dpu side")                   \
                                                                                \
  product(bool, DpuClientLivenessUpdateEnabled, false,                          \
          "Use this to enable liveness update for SnicGC on dpu side")          \
                                                                                \
  product(bool, NormalGCShmEnabled, false,                                      \
          "Use this to enable normal gc share memory")                          \
                                                                                \
  product(ccstr, SnicShareMemFilePath, "/mnt/hdd/lyh/share_mem",                \
          "snic shared file path")                                              \
                                                                                \
  product(ccstr, SnicShareRootsFilePath, "/mnt/hdd/lyh/share_roots",            \
          "snic share roots file path")                                         \
                                                                                \
  product(ccstr, SnicShmMemPath, "/share_heap",                                 \
          "snic shm path")                                                      \
                                                                                \
  product(ccstr, SnicShmRootsPath, "/share_roots",                              \
          "snic shm roots path")                                                \
                                                                                \
  product(ccstr, SnicShmRegionTamsPath, "/share_region_tams",                   \
          "snic shm region tams path")                                          \
                                                                                \
  product(ccstr, SnicShmBitmapPath, "/share_bitmap",                            \
          "snic shm bitmap path")                                               \
  product(ccstr, SnicShmLivenessPath, "/share_liveness",                        \
          "snic shm liveness path")                                             \
  product(ccstr, SnicShmControlPath, "/share_dgc_control",                      \
          "snic shm dgc control path for lock-free signaling")                  \
  product(bool, SnicShmLockFreeMarking, false,                                  \
          "Use SHM atomic signaling instead of TCP for marking sync")                                             \
  product(ccstr, SnicShmVirtualNodePath, "/virtual_node",                       \
          "snic shm virtual node path")                                         \
  product(ccstr, SnicShmGlobalPacerPath, "/share_global_pacer",                 \
          "snic shm global pacer path")                                         \
  product(ccstr, SnicCoorCCMTArgs, "0:0:500;4:0:500",                          \
          "Static seed for the CP-SAT marking-time model: "                    \
          "<CCMT_fallback>:<a>:<b_fallback>;<CCMT_dgc>:<a>:<b_dgc>, where "    \
          "marking_time(ms) = a * historyLiveness + b. With a=0 the model "    \
          "collapses to the constant b. The default 500/500 is a neutral "     \
          "seed; with +SnicCoorAdaptiveCCMT (default in the AE GC configs) "   \
          "EWMA replaces these values after a few cycles.")                    \
  product(bool, SnicCoorAdaptiveCCMT, false,                                   \
          "Adaptive: self-learn coor_ccmt_b values from measured marking times instead of using SnicCoorCCMTArgs as static")                                  \
  product(uintx, SnicCoorAdaptiveInitB, 500,                                   \
          "Adaptive: initial conservative b value (ms) before measurements accumulate")                                                                   \
  product(uintx, SnicCoorAdaptiveMinSamples, 3,                                \
          "Adaptive: warm-up sample count per CCMT before promoting DGC over fallback")                                                                  \
  product(double, SnicCoorAdaptiveSwitchHysteresis, 0.20,                      \
          "Adaptive: relative finish-time difference required to switch DGC<->fallback path")                                                            \
  product(bool, SnicDGCFaultHandling, true,                                    \
          "Enable runtime DGC fault detection: heartbeat liveness + cancellable timed waits on SHM marking/satb seqnos. When the SHM client is unresponsive, host falls back to local Shenandoah marking instead of blocking forever.") \
  product(uintx, SnicDGCHealthTimeoutMs, 2000,                                 \
          "Max age (ms) of SHM client heartbeat at GC start before host treats it as dead and falls back to local marking for this cycle.") \
  product(uintx, SnicDGCWaitSliceUs, 100000,                                   \
          "Per-iteration timed-wait slice (us) on SHM marking_done/satb_done seqnos. Bounds the time before the host re-checks cancelled_gc() so OOM-triggered alloc-failure can interrupt a stalled DGC marking.") \
  product(int, SnicGCTriggerOffset, 20,                                        \
          "snic gc trigger offset")                                            \
  product(int, SnicGCIntervalUnderEstimation, 50,                              \
          "snic gc interval under estimation")                                 \
  product(int, SnicDGCCCMT, 4,                                                 \
          "Concurrent marker count the host JVM advertises to the "            \
          "coordinator when scheduling DGC offload (matches the "              \
          "client's ShmClientMarkerNum / DpuClientMarkerNum).")                \
  product(int, SnicFallbackCCMT, 4,                                            \
          "Concurrent marker count the host JVM uses for local "               \
          "fallback marking when the coordinator returns INFEASIBLE "          \
          "or DGC is otherwise unavailable.")                                  \
  product(int, SnicCoorFallbackTrigger, 5,                                      \
          "Cycles of consecutive forceGC failure before the coordinator "      \
          "switches a host to fallback marking.")                              \
  product(int, SnicCoorMarkingTimeProvision, 50,                                \
          "Minimum marking-time provision (ms) the coordinator reserves "      \
          "when scheduling a DGC cycle, regardless of EWMA / static "          \
          "SnicCoorCCMTArgs estimates.")                                       \
  product(int, SnicCoorFrozenDGCUpperBound, 100,                                 \
          "snic coor frozen dgc threshold")                                     \
  product(int, SnicCoorFrozenDGCLowerBound, 20,                                 \
          "snic coor frozen dgc lower bound")                                   \
  product(bool, SnicGCGlobalPacer, false,                                       \
          "open global pacer")                                                  \
  product(int, SnicGCGlobalPacerInitTaxFree, 10,                                \
          "initial tax free rate for global pacer, in persent of free space")   \
  product(double, SnicGCGlobalPacerSurcharge, 1.1, EXPERIMENTAL,                \
          ...
          "the beginning of it.")                                               \
          range(1.0, 100.0)                                                     \
  product(double, SnicGCGlobalPacerMarkTimeSurcharge, 1.1, EXPERIMENTAL,        \
          "additional surcharge for mark time")                                 \
          range(0.9, 100.0)                                                     \
                                                                                \
  product(double, SnicGCGlobalPacerFreeHeadroomDiscount, 0.9, EXPERIMENTAL,     \
          "discount for free headroom")                                         \
          range(0.0, 1.1)                                                       \
  product(bool, SnicGCGlobalPacerForceGCEnabled, false,                         \
          "force gc enabled")                                                   \
                                                                                \
  product(int, ShmClientMarkerNum, 4,                                           \
          "number of markers for shm client")                                   \
  product(int, SnicShmSATBRootsHandleRounds, 2,                                 \
          "snic shm satb roots handle rounds")                                  \
  product(bool, SnicGCShareMemEnabled, true,                                    \
          "whether enable share mem for snic gc")                               \
                                                                                \
  product(uint64_t, SnicShmArenaBase, 0x1000000000ULL,                          \
          "Base VA for the SHM region arena. Both host and client reserve "    \
          "[SnicShmArenaBase, SnicShmArenaBase+SnicShmArenaSize) with "        \
          "PROT_NONE at JVM init so subsequent MAP_FIXED mmap of SHM regions " \
          "can't collide with libjvm/libc/stack. 0x1000000000 (64 GB VA) is "  \
          "safely above Java heap (0x500000000), CDS (0x800000000), and "      \
          "compressed class space, but below libjvm/stack (~0x7ffff0000000).") \
                                                                                \
  product(uint64_t, SnicShmArenaSize, 32ULL << 30,                              \
          "Size in bytes of the SHM region arena (default 32 GB). Must cover " \
          "the sum of: heap-mirror (Xmx), marking bitmap (Xmx/32), metaspace " \
          "VSNs (up to 4 GB each), roots / liveness / pacer / control. "       \
          "Default fits multi-GB Metaspace VSNs; raise if 'SHM arena "         \
          "exhausted' fires.")                                                 \
                                                                                \
  product(bool, SnicConcCopyRegion, true,                                       \
          "Use this to enable concurrent copy region on dpu side")              \
                                                                                \
  product(int, SnicSATBRootsSplitPartNum, 2,                                    \
          "Split SATB roots into several parts to reduce RPC size")             \
                                                                                \
  product(int, SnicSATBRootsForceDeltaNum, 20,                                  \
          "Number of delta SATB roots to force finish")                         \
                                                                                \
  product(ccstr, SNICAddr , "127.0.0.1",                                        \
          "Address of the RPC server")                                          \
                                                                                \
  product(int, SNICTransRegionGroupNum , 5,                                     \
          "Number of regions to copy in one group")                             \
                                                                                \
  product(ccstr, HostAddr , "127.0.0.1",                                        \
          "Address of the RPC server")                                          \
                                                                                        \
  product(ccstr, SnicCoorHostAddrPortList, "127.0.0.1:1234;127.0.0.1:1235",     \
        "List of host address and port")                                      \
                                                                                        \
  product(int, RPCPort , 9999,                                                  \
          "Port of the RPC server")                                             \
                                                                                \
  product(int, RDMAPort , 8888,                                                 \
          "Port of local RDMA server")                                          \
                                                                                \
  product(int, RDMAPortForCoor, 6666,                                           \
          "Port of RDMA server for coordinator")                                \
                                                                                \
  product(bool, MultiJVMForceBindCoreEnabled, false,                            \
          "Force bind GC threads to specific cores")                            \
                                                                                \
  product(ccstr, MultiJVMForceBindCore , "",                                    \
          "Cores used by GC threads")                                           \
                                                                                \
  product(ccstr, MutatorCores , "",                                             \
          "Cores used by mutators")                                             \
                                                                                \
  product(ccstr, GCThreadsCores , "",                                           \
          "Cores used by GC threads")                                           \
                                                                                \
  product(bool, DGCNoPenalty, false,                                            \
          "disable penalty for gc")                                             \
                                                                                \
  product(bool, SnicGCAlwaysDGC, false,                                         \
          "Always use DGC marking (bypass coordinator scheduling). "            \
          "Host self-triggers GC and always offloads marking to client. "       \
          "Requires SnicGCHost + SnicGCCoorHeuristic. No coordinator needed.")  \
                                                                                \
  product(bool, SnicGCRegionAddressTranslation, false,                          \
        "Use region-based address translation instead of fixed address mapping") \
                                                                                \
  product(size_t, SnicGCLocalMemoryPoolSize, 0,                                \
        "Size of local memory pool for region mapping (MB)") \
                                                                                \
  product(size_t, SnicGCLocalMemoryPoolMinAddress, 0x500000000,                 \
        "Minimum address of local memory pool for region mapping")              \
  product(size_t, SnicGCEvictionHotnessControl, 50,                         \
        "Hotness control for eviction, prevent region thrashing, in percentage")               \
  product(size_t, SnicGCEstimatedObjectSize, 256,                          \
        "Use this to enable marked data for eviction")                          \
  product(size_t, SnicGCEvictionMarkedDataUpdateThreshold, 1000,              \
        "Update threshold for marked data for eviction")                        \
