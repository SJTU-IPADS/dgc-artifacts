...
void before_exit(JavaThread* thread) {
  ...
  if(SnicGCHost) {
    ...
    if (!SnicGCShareMemEnabled) {
      log_debug(gc)("DGC LOG: do RPC 4");
      Universe::send_rpc(4, 2*sizeof(int), buffer);
    } else {
      if (SnicGCHost) {
        close(Universe::get_hostShareHeapFD());
        close(Universe::get_hostShareRootFD());
      }
      // Lock-free SHM hijacks the client's TCP RPC dispatch thread inside
      // runShmMarkingLoop (see shareMemSnicClient.cc case 5). Before sending
      // the TCP shutdown RPC, we must unstick that loop via the SHM control
      // struct, otherwise the client never gets back to the TCP socket and
      // the recv_int_ack() below deadlocks.
      if (SnicShmLockFreeMarking && Universe::get_shmDGCControl() != nullptr) {
        auto ctrl = Universe::get_shmDGCControl();
        Atomic::release_store(&ctrl->shutdown_requested, (unsigned long long)1);
        log_debug(gc)("DGC LOG: set shutdown_requested in SHM control before RPC 10");
      }
      log_debug(gc)("DGC LOG: send rpc 10 to share mem client");
      Universe::send_rpc(10, 2 * sizeof(int), buffer);
      Universe::recv_int_ack();
      // then unlink all shm spaces.
      Universe::shm_exit();
    }
    /*end @lyh*/
  }
