#!/bin/bash
# table3-shenandoah-specjbb-hbase-mix-run.sh
# Vanilla Shenandoah baseline for Table 3 SPECjbb+HBase mix workload.
# 2 SPECjbb backends + 2 HBase regionservers, NO coordinator / NO SHM clients.
# Each host JVM uses standalone Shenandoah on its own core slice.
...
# Baseline uses the SHM HBase install (script's TEST_MODE=local path)
HBASE_BASE_PATH="${HBASE_BASE_PATH:-$HOME_DIR/hbase-test/multi-regionserver-hbase-0}"
YCSB_BASE_PATH="$HOME_DIR/hbase-test/ycsb-0.18.0"

LOG_PATH="${LOG_PATH:-$(pwd)}"

# SPECjbb reads ./config/specjbb2015.props relative to cwd.
cd "$SCRIPT_DIR"
...
# Paper-fair baseline budget: 10 cores per host JVM.
PCORE_BASELINE=${PCORE_BASELINE:-10}
CCMT_BASELINE=${CCMT_BASELINE:-10}
...
# Even cores from low end (NUMA 0). host 0: 0,2,...,18; host 1: 20,22,...,38.
get_specjbb_backend_cpu_list() {
    local cores=$1
    local group_id=$2
    local cpus=""
    local start=$((group_id*cores*2))
...
# TxInjectors run on different even cores (skip backend ranges). Use later cores.
get_specjbb_txi_cpu_list() {
    local cores=$1
    local group_id=$2
    local cpus=""
    local start=$((40+cores*group_id*2))
    for ((i=0; i<cores; i++)); do
        cpus="$cpus$start,"
        start=$((start+2))
    done
...
PCORE=$PCORE_BASELINE
CCMT=$CCMT_BASELINE
GROUP_NUM=2
RPC_PORT=$((1024 + RANDOM % 64000))
log_path="$LOG_PATH/$heap_config_name/${CCMT}/${PCORE}/${GROUP_NUM}/baseline"
...
function run_specjbb_txi_and_backend {
    ...
    mkdir $log_path -p
    backend_log_file="${log_file_prefix}_specjbb_backend_baseline_${group_id}.txt"
    txi_log_file="${log_file_prefix}_specjbb_txi_baseline_${group_id}.txt"
    ...
    TXI_GC_OPTION="-XX:+UseShenandoahGC -XX:ParallelGCThreads=8 -XX:ConcGCThreads=8 -XX:+UnlockExperimentalVMOptions -XX:+UnlockDiagnosticVMOptions -XX:-ShenandoahVerify -XX:ShenandoahMaxSATBBufferFlushes=5 -Xmx8192m -Xms8192m"
    ...
    BACKEND_GC_OPTION="-XX:+UseShenandoahGC -XX:ParallelGCThreads=${PCORE} -XX:ConcGCThreads=${CCMT} -XX:+UnlockExperimentalVMOptions -XX:+UnlockDiagnosticVMOptions -XX:-ShenandoahVerify -XX:ShenandoahMaxSATBBufferFlushes=5"
    ...
    # Per-backend cwd so latency-with-start JAR's ./latency.txt doesn't collide.
    backend_cwd="${log_path}/backend_${group_id}_cwd"
    mkdir -p "$backend_cwd"
    cp -r "$SCRIPT_DIR/config" "$backend_cwd/" 2>/dev/null
    print_and_run "cd '$backend_cwd' && numactl -C $backend_cpu_list -m0 $java_cmd $gc_log_option $BACKEND_GC_OPTION $COMMON_FLAGS $BACKEND_TESTCASE_FLAGS" $backend_log_file $backend_log_file &
}
...
function run_specjbb_controller {
    ...
    log_file_prefix=$4
    mkdir $log_path -p
    controller_log_file="${log_file_prefix}_specjbb_controller_baseline.txt"
    controller_err_file="${log_file_prefix}_specjbb_controller_baseline.log"
    controller_cpu_list=$(get_specjbb_controller_cpu_list $PCORE)
    CONTROLLER_GC_OPTION="-XX:+UseShenandoahGC -XX:ParallelGCThreads=8 -XX:ConcGCThreads=8 -XX:+UnlockExperimentalVMOptions -XX:+UnlockDiagnosticVMOptions -XX:-ShenandoahVerify -XX:ShenandoahMaxSATBBufferFlushes=5"
...
function run_hbase_components {
    ...
    daemon_log_file="${log_file_prefix}_hbase_daemon_baseline.txt"
    regionserver_0_log_file="${log_file_prefix}_hbase_regionserver_baseline_0.txt"
    regionserver_1_log_file="${log_file_prefix}_hbase_regionserver_baseline_1.txt"
    log_file="${log_file_prefix}_BaselineHost.txt"
    ...
    # zookeeper
    run_cmd "bash $HBASE_BASE_PATH/bin/self-zookeeper-start --config $HBASE_BASE_PATH/conf zookeeper ${RPC_PORT} ${CCMT} ${CCMT} ${PCORE} ${hbase_heapsize} local start" >> $zookeeper_log_file 2>&1 &
    sleep 3
    # master
    run_cmd "bash $HBASE_BASE_PATH/bin/specjbb-hbase-mix-master-start --config $HBASE_BASE_PATH/conf master ${RPC_PORT} ${CCMT} ${CCMT} ${PCORE} ${hbase_heapsize} local start" >> $daemon_log_file 2>&1 &
    sleep 3
    # regionserver 0 (TEST_MODE=local → vanilla Shenandoah, NOT SnicGC)
    run_cmd "bash $HBASE_BASE_PATH/bin/specjbb-hbase-mix-regionserver-start --config $HBASE_BASE_PATH/conf regionserver ${RPC_PORT} ${CCMT} ${CCMT} ${PCORE} ${hbase_heapsize} 2 9999 ${CCMT} local -Dhbase.regionserver.port=$region_server_port -Dhbase.regionserver.info.port=$((region_server_port+10)) start" >> $regionserver_0_log_file 2>&1 &
    sleep 3
    # regionserver 1
    run_cmd "bash $HBASE_BASE_PATH/bin/specjbb-hbase-mix-regionserver-start --config $HBASE_BASE_PATH/conf regionserver ${RPC_PORT} ${CCMT} ${CCMT} ${PCORE} ${hbase_heapsize} 3 9999 ${CCMT} local -Dhbase.regionserver.port=$((region_server_port+1)) -Dhbase.regionserver.info.port=$((region_server_port+11)) start" >> $regionserver_1_log_file 2>&1 &
    sleep 3
    init_hbase_for_ycsb
}
...
function run_hbase_ycsb {
    ...
    log_file="${log_file_prefix}_BaselineHost.txt"
    loop_time=0
    hbase_ycsb_queries_load_and_warmup $log_file_prefix $loop_time BaselineHost $daemon_log_file $PCORE $HBASE_IR_RATE $HBASE_YCSB_WORKLOAD
    for((i=0;i<2;i++))
    do
        run_specjbb_txi_and_backend "shenandoah" "$JAVA_HOME/bin/java" "$LOG_OPTION" "$log_file_prefix" $i
    done
    sleep 3
    run_specjbb_controller "shenandoah" "$JAVA_HOME/bin/java" "$LOG_OPTION" "$log_file_prefix" &
    controller_pid=$!
    sleep 1
    echo "start to wait finish of SPECjbb warmup..."
    target_specjbb_controller_log_file="${log_file_prefix}_specjbb_controller_baseline.txt"
    ...
    hbase_ycsb_queries_reload_and_run $log_file_prefix $loop_time BaselineHost $daemon_log_file $PCORE $HBASE_IR_RATE $HBASE_YCSB_WORKLOAD
...
# Start HBase
run_hbase_components "shenandoah" "$JAVA_HOME/bin/java" "$LOG_OPTION" "$log_file_prefix"
# YCSB load + warmup → SPECjbb up → YCSB run + wait
run_hbase_ycsb "shenandoah" "$JAVA_HOME/bin/java" "$LOG_OPTION" "$log_file_prefix"
